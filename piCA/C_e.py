...
def compute(X,k,amuse,circ,norm):
    ...
    if not(amuse or circ):
        raise ValueError("We cannot compute Ce matrix")
    '''
    ...
    Cs = np.empty( (int(0.5*sensors*(sensors+1)),fft_samples),dtype = np.float64)
    l = 0 
    ...
    Ce = np.zeros((L,sensors,sensors) , dtype=np.float64)
    m = 0
    for i in range(sensors):
        for j in range(i,sensors):
            # Compute F(cij) = Re[F{Xf(i)}*F{Xfc(j)}]
